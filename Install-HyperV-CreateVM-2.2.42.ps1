...
.PARAMETERS
  ...
  EXAMPLE
    ...
    .\Install-HyperV-CreateVM-2.2.38.ps1 -Name <Name> -CPUCount <1-6> -Autopilot -PowerOn -ProductKey
...
$IsoPath = Get-ChildItem -Path "$VirtualMedia" -Filter *.iso -File -Recurse -ErrorAction SilentlyContinue |
...
$osName = (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion").ProductName
  ...
  if ($osName -like "*Windows 11*") {
    Write-Host "$(Get-TimeStamp) Operating System: Microsoft Windows 11" -ForegroundColor Green
    ...
    Write-Host "Windows 11 Product Key = $value " -ForegroundColor Cyan

  } else {
    Write-Host "Operating Sytem: Is Not Windows 11. A Windows 11 Product Key will be required" -ForegroundColor Red
    Write-Host "during first boot/OOBE setup." -ForegroundColor Red
  } 
}

Write-Host "NETWORK DRIVERS" -ForegroundColor Yellow
Write-Host "Hyper-V binds to the local physical Nic on the host computer. Because of" -ForegroundColor Yellow
Write-Host "this drivers maybe need to be added to the Windows Installation Media ISO." -ForegroundColor Yellow
Write-Host "This will be evident if the VM cannot communicate with Entra, Intune or Autopilot" -ForegroundColor Yellow
Write-Host "or prompts for Wifi drivers. A script has been create to inject drivers into the" -ForegroundColor Yellow
Write-Host "Windows Installation Media" -ForegroundColor Yellow
Write-Host ""
Write-Host "Get the powershell script here:" -ForegroundColor Cyan
Write-Host "https://github.com/Metropolis-Pt3" -ForegroundColor Cyan
Write-Host ""
Write-Host "------------------------------------------------------------------------------"
Write-Host ""
Write-Host "AUTOPILOTINFO:" -ForegroundColor Yellow
Write-Host "Once Windows has been installed, during first boot before selecting" -ForegroundColor Yellow
Write-Host "languages or regions, press SHIFT + fn + F10 to being up the command prompt." -ForegroundColor Yellow
Write-Host "The computer hash of the device will need to be gathered and imported into" -ForegroundColor Yellow
Write-Host "Intune for autopilot assignment. PowerShell, PowerShell-Ise and Explorer can" -ForegroundColor Yellow
Write-Host "be run from this command prompt. The following Powershell script will generate" -ForegroundColor Yellow
Write-Host "a file that can be imported into Intune." -ForegroundColor Yellow
Write-Host ""
...
Write-Host "In the virtual Machine Directory (C:\Temp\Virtual\<ComputerName>) is a" -ForegroundColor Yellow
Write-Host "folder called (Autopilot) that includes a file that can be used for the" -ForegroundColor Yellow
Write-Host "corporate identifier. This will ensure Intune Autopilot considers the" -ForegroundColor Yellow
Write-Host "Hyper-V virtual machine a (Corporate) device, not personal." -ForegroundColor Green
